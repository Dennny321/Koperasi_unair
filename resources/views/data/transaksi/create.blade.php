...
/* Keranjang Items - FIXED: ini yang penting untuk scroll */
.pos-keranjang {
    flex: 1;
    overflow-y: auto;
    overflow-x: hidden;
...
<div class="scanner-modal" id="scannerModal">
    <div class="scanner-modal-content">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
            <div>
                ...
                <p style="font-size:12px; color:var(--text-secondary); margin-top:4px;">
                    Scanner tetap aktif — scan berulang kali
                </p>
            </div>
        ...
        <div id="scannerResult" style="margin-top:12px; padding:10px 14px; border-radius:8px; display:none; font-weight:600; font-size:14px; transition:all .2s;">
            <i class="fas fa-circle-notch fa-spin"></i> <span id="scannerResultText"></span>
        </div>
        <div style="margin-top:14px; display:flex; justify-content:space-between; align-items:center;">
            ...
            <button type="button" onclick="tutupScanner()" class="btn btn-warning btn-sm">
                <i class="fas fa-check"></i> Selesai Scan
            </button>
        </div>
    </div>
</div>
...
function renderKeranjang() {
    ...
    // Hapus semua item kecuali elemen empty, lalu render ulang
...
// Scanner tetap aktif setelah scan — bisa scan banyak produk berturut-turut
// ===========================
let scannerBusy = false; // mencegah double-scan saat proses AJAX
...
function bukaScanner() {
    ...
    scanner.start(
        ...
        async (decodedText) => {
            ...
            document.getElementById('scannerResult').style.background = '#fef3c7';
            document.getElementById('scannerResult').style.color = '#92400e';

            try {
                const res  = await fetch(`{{ route($rp.'.transaksi.cari-produk') }}?kode=${encodeURIComponent(decodedText)}`);
                const data = await res.json();
                ...
                if (data.success) {
                    ...
                    // Feedback sukses di modal
                    document.getElementById('scannerResultText').textContent = '✅ ' + data.produk.nama + ' ditambahkan!';
                    document.getElementById('scannerResult').style.background = '#d1fae5';
                    document.getElementById('scannerResult').style.color = '#065f46';

                    // Bunyi bip (opsional — browser support terbatas)
                    try {
                    ...
                    document.getElementById('scannerResultText').textContent = '❌ ' + data.message;
                    document.getElementById('scannerResult').style.background = '#fee2e2';
                    document.getElementById('scannerResult').style.color = '#991b1b';
                    showToast(data.message, 'error');
                }
            } catch (e) {
                document.getElementById('scannerResultText').textContent = '❌ Gagal terhubung ke server';
                showToast('Gagal mencari produk', 'error');
            }

            // Biarkan scanner tetap aktif, reset busy setelah 1.5 detik
            setTimeout(() => {
                scannerBusy = false;
                document.getElementById('scannerResult').style.display = 'none';
            }, 1500);
        },
        () => {} // error callback (abaikan frame gagal)
    ).catch(() => {
...
// Scanner ketik kode lalu tekan Enter → otomatis tambah ke keranjang
// ===========================
(function() {
    let barcodeBuffer = '';
    ...
    document.addEventListener('keydown', function(e) {
        // Abaikan jika fokus di input/textarea (kecuali search produk)
        const tag = document.activeElement.tagName;
        const id  = document.activeElement.id;
        if ((tag === 'INPUT' || tag === 'TEXTAREA') && id !== 'produkSearch') return;
        // Abaikan jika modal scanner kamera sedang terbuka
        if (document.getElementById('scannerModal').classList.contains('active')) return;

        if (e.key === 'Enter') {
            // Enter = akhir dari scan barcode
            if (barcodeBuffer.length >= 3) {
                prosesBarcodeDariKeyboard(barcodeBuffer.trim());
            }
            barcodeBuffer = '';
    ...
    async function prosesBarcodeDariKeyboard(kode) {
        // Tampilkan indikator scan
        showToast('🔍 Mencari: ' + kode + '...', 'success');
        try {
            const res  = await fetch(`{{ route($rp.'.transaksi.cari-produk') }}?kode=${encodeURIComponent(kode)}`);
            const data = await res.json();
            if (data.success) {
                const el = document.createElement('div');
                el.dataset.id     = data.produk.id;
