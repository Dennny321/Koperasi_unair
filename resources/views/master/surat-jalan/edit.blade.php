...
@php
    ...
    $existingDetail = $suratJalan->detail->map(function ($d) {
        return [
...
<div class="content-header">
    <div>
        <h2 style="color: var(--primary); font-size: 28px; margin-bottom: 4px;">Edit Surat Jalan</h2>
        <p style="color: var(--text-secondary);">{{ $suratJalan->no_surat }}</p>
    </div>
    <a href="{{ route($rp.'.surat-jalan.show', $suratJalan) }}" class="btn btn-outline-secondary">
        <i class="fas fa-arrow-left me-1"></i> Kembali
    </a>
</div>

@if($errors->any())
<div style="background:#fee2e2; border:1px solid #fca5a5; border-radius:10px; padding:14px 18px; margin-bottom:16px; color:#991b1b;">
    <i class="fas fa-exclamation-circle me-2"></i>
    <ul class="mb-0 ps-3">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul>
</div>
@endif

<form action="{{ route($rp.'.surat-jalan.update', $suratJalan) }}" method="POST" id="formSuratJalan">
@csrf @method('PUT')
<div class="row g-4">
    {{-- INFO SURAT --}}
    <div class="col-lg-5">
        <div class="table-container" style="padding:24px;">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" value="{{ $suratJalan->no_surat }}" disabled>
            </div>

            <div class="mb-3" style="margin-top: 10px">
                <label class="form-label fw-semibold">No. Transaksi</label>
                <input type="text" class="form-control" value="{{ $suratJalan->no_transaksi }}" disabled>
            </div>
            ...
            <div class="mb-3" style="margin-top: 10px">
                ...
                <input type="text" name="tujuan" class="form-control @error('tujuan') is-invalid @enderror"
                    value="{{ old('tujuan', $suratJalan->tujuan) }}" placeholder="Nama instansi / alamat tujuan" required>
                @error('tujuan')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="mb-3" style="margin-top: 10px">
                <label class="form-label fw-semibold">Keterangan</label>
                <textarea name="keterangan" class="form-control" rows="3" placeholder="Catatan tambahan (opsional)">{{ old('keterangan', $suratJalan->keterangan) }}</textarea>
            </div>
        </div>
    </div>
        ...
        <div class="table-container" style="padding:24px;">
            ...
            <div class="table-responsive mb-3">
                <table class="table table-bordered" id="tabelProduk">
                    ...
                    <tbody id="bodyProduk">
                        {{-- baris diisi via JS dari data existing --}}
                    </tbody>
                    <tfoot>
...
<script>
    ...
    // Data barang existing dari server (dipakai untuk isi baris awal)
    const existingDetail = @json($existingDetail);

    function tambahBaris(prefill = null) {
        ...
        document.getElementById('bodyProduk').appendChild(actual);
        bindBaris(actual, prefill);
        hitungTotal();
    }

    function bindBaris(tr, prefill = null) {
        ...
        // Prefill data existing (mode edit)
        if (prefill) {
            $(selProduk).val(prefill.id_produk).trigger('change');
            const opt = selProduk.options[selProduk.selectedIndex];
            inpJumlah.value = prefill.jumlah;
            inpHarga.value = prefill.harga_satuan;
            if (opt) {
    ...
    // Isi baris dari data existing, atau 1 baris kosong jika tidak ada
    if (existingDetail.length > 0) {
        existingDetail.forEach(item => tambahBaris(item));
    } else {
        tambahBaris();
    }
</script>
