...
@section('breadcrumb', 'Master / Produk / Edit')
@section('page-title', 'Edit Produk')

@section('content')
<div class="content-header">
    <div>
        <h2 style="color: var(--primary); font-size: 28px; margin-bottom: 4px;">Edit Produk</h2>
        <p style="color: var(--text-secondary);">Update informasi produk yang sudah ada</p>
    </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px;">
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="kode_produk" id="kode_produk" class="form-control @error('kode_produk') is-invalid @enderror" value="{{ old('kode_produk', $produk->kode_produk) }}" placeholder="Ketik kode atau scan dengan alat barcode scanner" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $produk->nama) }}" placeholder="Masukkan nama produk" required>
                        @error('nama')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="harga" class="form-control @error('harga') is-invalid @enderror" value="{{ old('harga', $produk->harga) }}" placeholder="0" step="0.01" min="0" required>
                        @error('harga')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
...
<small class="form-text text-muted"><i class="fas fa-lock"></i> Stok dikelola otomatis melalui transaksi restock</small>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stok_minimum" class="form-control @error('stok_minimum') is-invalid @enderror" value="{{ old('stok_minimum', $produk->stok_minimum) }}" placeholder="0" min="0" required>
                        @error('stok_minimum')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="satuan" class="form-control @error('satuan') is-invalid @enderror" value="{{ old('satuan', $produk->satuan) }}" placeholder="Contoh: Pcs, Kg, Liter" required>
                        @error('satuan')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="form-group">
                ...
                <div style="margin-bottom: 16px;">
                    <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 8px;">Foto Saat Ini:</p>
                    <img src="{{ asset('storage/' . $produk->foto) }}" alt="{{ $produk->nama }}" style="max-width: 200px; border-radius: 10px; border: 2px solid var(--border-color);">
                </div>
                @endif
                ...
                <div class="form-file">
                    ...
                    <label for="foto" class="form-file-label">
                        <div id="preview-container">
                            ...
                            <p style="margin: 8px 0 4px; font-weight: 600; color: var(--text-main);">Klik untuk upload foto baru</p>
                            <p style="font-size: 12px; color: var(--text-secondary);">Format: JPG, PNG (Max. 2MB) - Kosongkan jika tidak ingin mengubah</p>
                        </div>
                    </label>
                </div>
                @error('foto')
                ...
                <div id="image-preview" style="margin-top: 16px; display: none;">
                    <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 8px;">Preview Foto Baru:</p>
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                document.getElementById('preview-container').innerHTML = `
                    ...
                    <p style="margin: 8px 0 4px; font-weight: 600; color: var(--success);">Foto baru berhasil dipilih!</p>
